...
export default function EditReviewScreen() {
  ...
  const [review, setReview] = useState(reviews.find(r => r.id === id));
  ...
  useEffect(() => {
    ...
    if (review.targetType === 'kitchen') {
      const kitchen = kitchens.find(k => k.id === review.targetId);
      if (kitchen) {
        setTargetName(kitchen.name);
        setTargetImage(kitchen.image);
      }
    } else if (review.targetType === 'dish') {
      const dish = dishes.find(d => d.id === review.targetId);
      if (dish) {
      ...
      Alert.alert(
        ...
        [
          { 
            ...
            onPress: () => {
              if (review.targetType === 'kitchen') {
                router.push(`/reviews/kitchen/${review.targetId}`);
              } else {
                router.push(`/reviews/dish/${review.targetId}`);
              }
            }
          }
        ]
        ...
        <View style={styles.targetInfo}>
          ...
          <View>
            <Text style={styles.targetType}>
              {review.targetType === 'kitchen' ? 'Kitchen Review' : 'Dish Review'}
            </Text>
